...
function printReport() {
    if (document.getElementById('clients-export-meta')) {
        printClientsList();
        return;
    }
...
function collectClientsFromTable() {
    const rows = document.querySelectorAll('tr.client-export-row');
    return Array.from(rows).map(function(row) {
        return {
            id: row.getAttribute('data-export-id') || '',
            categoria: row.getAttribute('data-export-categoria') || '',
            nombre: row.getAttribute('data-export-nombre') || '',
            telefono: row.getAttribute('data-export-telefono') || '',
            direccion: row.getAttribute('data-export-direccion') || '',
            barrio: row.getAttribute('data-export-barrio') || '',
            ciudad: row.getAttribute('data-export-ciudad') || '',
            ipRouter: row.getAttribute('data-export-ip-router') || '',
            ipAntea: row.getAttribute('data-export-ip-antea') || '',
            incidencias: row.getAttribute('data-export-incidencias') || '0',
        };
    });
}

function buildClientsPrintHtml(options) {
    const {
        clients,
        filters,
        stats,
    ...
    let tableContent = '';
    if (clients.length > 0) {
        const bodyRows = clients.map(function(client) {
            return '<tr>'
                + `<td><strong>${escapeHtml(client.id)}</strong></td>`
                + `<td>${escapeHtml(client.categoria)}</td>`
                + `<td>${escapeHtml(client.nombre)}</td>`
                + `<td>${escapeHtml(client.telefono)}</td>`
                + `<td class="col-direccion">${escapeHtml(client.direccion)}</td>`
                + `<td>${escapeHtml(client.barrio)}</td>`
                + `<td>${escapeHtml(client.ciudad)}</td>`
                + `<td>${escapeHtml(client.ipRouter)}</td>`
                + `<td>${escapeHtml(client.ipAntea)}</td>`
                + `<td>${escapeHtml(client.incidencias)}</td>`
                + '</tr>';
        }).join('');
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Listado de Clientes - CONNEXIA</title>
    <style>
        :root {
            --brand: #c82333;
            --ink: #212529;
        ...
        * { box-sizing: border-box; }
        ...
        @media print {
            ...
            .filters, .stats-wrap { break-inside: avoid; }
            th, td { font-size: 9px; padding: 4px 3px; }
        }
    </style>
</head>
<body>
    ...
    <div class="filters">
        ...
        <div class="filters-grid">
            <div><span>Búsqueda:</span> ${escapeHtml(filters.busqueda)}</div>
            <div><span>Barrio:</span> ${escapeHtml(filters.barrio)}</div>
            <div><span>Ciudad:</span> ${escapeHtml(filters.ciudad)}</div>
            <div><span>Categoría:</span> ${escapeHtml(filters.categoria)}</div>
            <div><span>Paginación:</span> ${escapeHtml(filters.paginacion)}</div>
        </div>
    </div>
    ...
    <div class="stats-wrap">
        ...
        <div class="stats">
            ...
            <div class="stat-card">
                <div class="stat-number">${stats.corporativo}</div>
                <div class="stat-label">Corporativo</div>
            </div>
            <div class="stat-card">
                <div class="stat-number">${stats.particular}</div>
...
function printClientsList() {
    try {
        const searchQuery = document.getElementById('search') ? document.getElementById('search').value.trim() : '';
        const meta = document.getElementById('clients-export-meta');
        const page = meta ? meta.getAttribute('data-page') || '1' : '1';
        const pages = meta ? meta.getAttribute('data-pages') || '1' : '1';
        const perPage = meta ? meta.getAttribute('data-per-page') || '50' : '50';

        const clients = collectClientsFromTable();
        const stats = {
            total: clients.length,
            corporativo: clients.filter(function(c) { return c.categoria === 'Corporativo'; }).length,
            particular: clients.filter(function(c) { return c.categoria === 'Particular'; }).length,
        };

        const hasActiveFilters = Boolean(
            searchQuery
            || (document.getElementById('ville') && document.getElementById('ville').value)
            || (document.getElementById('ciudad') && document.getElementById('ciudad').value)
            || (document.getElementById('categoria') && document.getElementById('categoria').value)
        );

        const filters = {
            busqueda: searchQuery || '(ninguna)',
            barrio: selectedOptionLabel('ville', 'Todos'),
            ciudad: selectedOptionLabel('ciudad', 'Todas'),
            categoria: selectedOptionLabel('categoria', 'Todas'),
            paginacion: `Página ${page} de ${pages} — ${perPage} registros por página`,
        };
        ...
        printWindow.document.write(buildClientsPrintHtml({
            clients: clients,
            filters: filters,
            stats: stats,
            generatedAt: formatDateTimeEs(new Date()),
